class Admin::FormRequestsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @form_request.update(form_request_params)
        LegalForm.create(legal_form_title: @form_request.title, legal_form_content: @form_request.content)
        
        @form_request.user.messages.create(content: "The " + @form_request.title + " form is now available at Forms Section.", is_lea_response: true)
